...
#let ongoing_research = cv_section_list("ONGOING RESEARCH", 
  (
    cv_entry(
      entry_heading( l: [Closing the Intent-To-Behavior Gap via Fulfillment Priority Logic], r: [submitted to iros] ),
    ...
    cv_entry(
      entry_heading( l: [Anchored Learning for On-the-Fly Adaptation], r: [submitting to RA-L] ),
    ...
    cv_entry(
      entry_heading( l: [Boolean Function Differentiation for Efficient Backpropagation], r: [ongoing] ),
      [We propose a method for efficiently computing the gradient of Boolean functions. Our method avoids the standard pitfalls of lack of gradient information by constructing computational graphs effectively. #links(labeled("github", link("https://github.com/bmabsout/discrete_learning")[bmabsout/discrete_learning]))]
    ),
    cv_entry(
